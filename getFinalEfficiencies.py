...
for signal, nprocessed, istrategy in signals:
    ...
    #    print signal, round(eff * 100, 2), round(deff * 100,2)
        ...
        x[istrategy+icut*2].append(signal.split("_")[3].split("m")[1])
...
c1 = TCanvas()
legend = TLegend(0.15, 0.74, 0.50, 0.94, "X#rightarrow HH #rightarrow #gamma#gammab#bar{b}")
...
gr2.Draw("LP");
gr3.Draw("LP");
legend.AddEntry(gr0.GetName(), "High+Medium Purity - Low mass - fit m_{#gamma#gamma}", "lp")
legend.AddEntry(gr1.GetName(), "High+Medium Purity - High mass - fit m_{#gamma#gamma jj}", "lp")
legend.AddEntry(gr2.GetName(), "High Purity - Low mass - fit m_{#gamma#gamma}", "lp")
legend.AddEntry(gr3.GetName(), "High Purity - High mass - fit m_{#gamma#gamma jj}", "lp")
legend.Draw()
latexLabel = TLatex()
latexLabel.SetTextSize(0.03)
latexLabel.SetNDC()
latexLabel.DrawLatex(0.15, 0.96, "CMS Preliminary Simulation")

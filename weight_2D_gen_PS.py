...
plots.append(["gr_PS_costhetastar_CS_pt_2D", "gr_hbb_p4_pt:fabs(gr_hbbhgg_costhetastar_CS)", "", 1., "(20, 0, 1, 250, 0, 500)", "|cos#theta*|^{#hh}", "", "", ""])
...
#        if norm == 1.:
            h.GetYaxis().SetTitle( "pT^{#hbb}" + str(((xhigh - xlow) / xnbin)) + " " )
#        else:
...
#    latexLabel.DrawLatex(.43, .72, "w = w_#lambda x w_hadronization")
    ...
    if iplot == 0:
        fileOption = "RECREATE"
    fileOut = TFile("weights_2D_gen_PS.root", fileOption)
    for ihisto, histo in enumerate(hDivided):
        print histo.GetName()
